...
\iffalse
- Signed clique-width and consensus treewidth are incomparable \cite{DBLP:conf/sat/GanianS17}.

- For any formula $F$ with clauses of width at most $d$ and without pure literals, it holds that \textbf{itw}$(F) \leq (d + 1) \cdot (\textbf{conflict-tw}(F) + 1)$ \cite{DBLP:conf/sat/GanianS17}.\\

- Bounded twin-width. ... most notably, it is upper-bounded on graphs of bounded clique-width \cite{DBLP:conf/sat/GanianPSSS22}.

- Let a graph $G$ have a clique-width of $d$. Then $G$ has a twin-width of at most $2d$. Moreover, if $G$ is a signed graph with a signed clique-width of $d$, the signed twin-width of $G$ is at most $2d$ \cite{DBLP:conf/sat/GanianPSSS22}.\\

- Bounded treewidth implies bounded clique-width. Treewidth $k$ to clique-width at most $2^{k+1}+1$ \cite{DBLP:journals/dam/FischerMR08}.\\

- if clique-width is bounded, then rank-width is bounded, and vice versa \cite{DBLP:journals/jct/OumS06}.\\

- For any CNF formula $F$ over $m$ clauses, the ps-width of $F$ is no larger than $m^{\textbf{mimw}(I(F))}$ (incidence graph) \cite{DBLP:conf/sat/SaetherTV14}.

- MIM-width of any graph $G$ is at most the clique-width of $G$ \cite{DBLP:conf/sat/SaetherTV14}.\\

- rw(G) $\leq$ cw(G) $\leq$ $2^{rw(G)+1}-1$. \cite{DBLP:journals/fuin/GanianHO13}.

- cw(G) $\leq$ $2^{rw(G)/2-1}$. \cite{DBLP:journals/fuin/GanianHO13}.

- rw(G) $\leq$ bw(G) $\leq$ tw(G)+1. \cite{DBLP:journals/fuin/GanianHO13, DBLP:journals/jgt/Oum08}.
\fi
